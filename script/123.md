...
#61 - #70
Also because in here,
not all the Component information is available yet.
For example, if you lock the text property here,
which is set as an input here,
so which receives the data from outside the Component,
if you log it here in the constructor,
you will see that if you reload
and render this Component again,
you get undefined initially
because the input value won't have been received
in the constructor yet.

또한 여기서는
아직 모든 컴포넌트 정보를 사용할 수 없습니다.
예를 들어, 여기서 입력으로 설정되고
컴포넌트 외부에서 데이터를 받는
text 속성을 잠그면,
constructor에서 이를 로그하면,
다시 로드하고
이 컴포넌트를 다시 렌더링할 때
처음에는 undefined를 얻게 됩니다.
이는 constructor에서
아직 입력 값을 받지 않았기 때문입니다.

#71 - #80
So if you would need to do some initialization work based
on some input values, you couldn't do it here.
And indeed, it is considered a bad practice
to do any complex Component initialization
or set up work in this constructor function.
You should only do basic class initialization work here,
like set some initial properties.
But if you need to do anything more complex,
like for example send an HTDP request,
which we will do later in this course,
you should do that in ngOnInit.
And here you also have access to any input property values.

따라서 입력 값을 기반으로
초기화 작업을 해야 한다면, 여기서는 할 수 없습니다.
실제로 이 constructor 함수에서
복잡한 컴포넌트 초기화나
설정 작업을 하는 것은 나쁜 관행으로 간주됩니다.
여기서는 초기 속성을 설정하는 것과 같은
기본적인 클래스 초기화 작업만 해야 합니다.
하지만 나중에 이 과정에서 할
HTTP 요청을 보내는 것과 같은
더 복잡한 작업이 필요한 경우,
ngOnInit에서 수행해야 합니다.
그리고 여기서는 모든 입력 속성 값에 접근할 수 있습니다.

#81 - #85
So if you log this text property here in ngOnInit,
you will see that if you log this all again,
once ngOnInit executes,
this initial text value is available.
So this text property value is available.
And it is therefore ngOnInit where you should do
your main Component initialization work.

따라서 ngOnInit에서 이 text 속성을 로그하면,
모두 다시 로그할 때,
ngOnInit이 실행되면
이 초기 텍스트 값을 사용할 수 있게 됩니다.
즉 이 텍스트 속성 값을 사용할 수 있습니다.
그래서 주요 컴포넌트 초기화 작업은
ngOnInit에서 수행해야 합니다.
...
이전에 설명했듯이 ngOnChanges는
입력 값이 변경될 때마다 실행됩니다.
그리고 이 훅을 사용하여 예를 들어,
컴포넌트 내부 상태를 업데이트할 수 있습니다,
즉 입력 값이 변경될 때
변경해야 할 수 있는
관리해야 할 다른 속성을 업데이트할 수 있습니다.
여기서 그렇게 할 수 있습니다.
그리고 이러한 다른 훅들이 있습니다.
이것들은 무엇에 관한 것일까요?
...
ngDoCheck는 Angular의 변경 감지 메커니즘과
관련이 있습니다.
결국 ngDoCheck는
UI 업데이트가 필요할 수 있다고 Angular가 판단할 때마다
Angular에 의해 호출됩니다.
그리고 여기에 ContentInit, ContentChecked,
ViewInit 및 ViewChecked 훅 메서드들이 있습니다.

#96 - #100
And what are those about?
Well, first it's important to understand what the difference
between content and view is,
...
이것들은 무엇에 관한 것일까요?
먼저, content와 view의 차이점을 이해하고
이것이 Angular 컴포넌트와
어떤 관련이 있는지 이해하는 것이 중요합니다.
view부터 시작해보겠습니다.
결국, Angular 컴포넌트의 view는
말하자면 그것의 템플릿입니다.
따라서 그것이 컴포넌트의 view이고,
여기 이 템플릿 파일에 있는
모든 요소들이 그 Angular view의 일부입니다.
...
these hooks too often, but if you do have some logic
that should execute whenever the projected content
might have changed,
you could use these hooks to run that logic.

그리고 이 훅들은 단순히
투영된 content가 초기화되면 실행됩니다.
따라서 여기서 해당 content에 접근하고
그것에 대한 정보를 얻을 수 있습니다.
그리고 여기 이 checked 훅은
Angular의 변경 감지 메커니즘에 의해
content가 확인될 때마다 실행됩니다.
바로 말씀드리자면 이러한 훅들을
자주 사용하지는 않을 것입니다만,
투영된 content가 변경될 때마다
실행되어야 하는 로직이 있다면,
해당 로직을 실행하기 위해 이 훅들을 사용할 수 있습니다.
...
이제 ViewInit과 ViewChecked도 같은 방식이지만
이제는 이 컴포넌트의 템플릿에 대한 것입니다.
따라서 여기서는 투영된 content에 관한 것이 아니라
이 컴포넌트에 속하는 템플릿에
관한 것입니다.
여기에는 투영된 content도 포함되지만
템플릿에 있는
다른 모든 요소들도 포함됩니다.
...
그리고 여기서도 마찬가지로, 첫 번째 훅인
ViewInit 훅은 Angular가 이 컴포넌트의 view
초기화를 완료하면 실행됩니다.
즉, 실제 DOM에 렌더링이 완료되면
실행된다고 할 수 있습니다.
그리고 ngAfterViewChecked는
Angular가 이 컴포넌트의 view에 대해
변경 감지를 수행할 때마다 실행됩니다,
즉 가능한 데이터 변경으로 인해
UI 업데이트가 필요한지 확인할 때 실행됩니다.
...
다시 말씀드리지만, 이러한 훅들을 자주 사용하지는 않겠지만,
이것이 이 훅들이 무엇에 관한 것이고
언제 호출되는지에 대한 설명입니다.
그리고 ngOnDestroy는
이 컴포넌트 인스턴스가
제거되기 직전에 호출됩니다,
예를 들어 컴포넌트가 조건부로 렌더링되고
그 조건이 더 이상 충족되지 않을 때입니다.
이것이 이러한 라이프사이클 훅들에 관한 설명입니다.
...
이제 공식 문서에서
여기에서 언급된 다른 메서드들도 보게 될 것이고,
이 섹션 후반부에서 다시 다루게 될 것입니다.
그리고 일반적으로 이 과정 전반에 걸쳐
이러한 훅들 중 일부,
가장 중요한 훅들을 실제로 계속 보게 될 것입니다.
지금은 이러한 훅들에 대한 첫 걸음을 떼기 위한
심층 탐구였으며, 이것들이 무엇에 관한 것인지
첫 인상을 가질 수 있게 했습니다.
